...
export default {
  ...
  data() {
    return {
      ...
      tableColumn: [
        { field: 'head', title: '头数范围', width: 180, editRender: { name: 'input' }},
        { field: 'stitches', title: '针数范围', width: 180, editRender: { name: 'input' }},
        { field: 'price', title: '工价', width: 80, editRender: { name: '$input', type: 'number', props: { type: 'number' }}},
        { field: 'unit', title: '单位', width: 80, editRender: { name: '$select', options: [{ label: '元/车', value: '元/车' }, { label: '元/千针', value: '元/千针' }], props: { placeholder: '请选择单位' }}},
        { field: 'createTime', title: '创建时间', width: 200, editRender: { name: '$input', props: { type: 'datetime' }}},
        { title: '操作', width: 170, slots: { default: 'operate' }}
      ],
      editRules: {
        head: [
          { required: true, message: '必须包含头数范围' }
        ],
        stitches: [
          { required: true, message: '必须包含针数范围' }
        ],
        price: [
          { required: true, message: '必须包含工价' }
        ],
        unit: [
          { required: true, message: '必须包含单位' }
        ]
      },
      pickerOptions: {
        shortcuts: [{
  ...
  methods: {
    ...
    async editClosedEvent({ row, column }) {
      ...
      if ($table.isUpdateByRow(row, field)) {
        ...
        upData({ 'collection': 'price', 'id': newRow._id, 'data': newRow }).then(response => {
          console.log(response)
          // 局部更新单元格为已保存状态
      ...
      if ($table.isInsertByRow(row)) {
        console.log(row)
        if (row.price && row.head && row.stitches && row.unit) {
          this.createData(row)
        } else {
          this.$message({
            type: 'error',
